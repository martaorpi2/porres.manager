@php
    ...
    // role_admin_institucion solo puede eliminar sus propias calificaciones
    if ($user && $user->hasRole('role_admin_institucion', 'backpack')) {
        $canDelete = $entry->rated_by == $user->id;
    } else {
        // Otros roles pueden eliminar si tienen acceso
        $canDelete = $crud->hasAccess('delete', $entry);
    }
@endphp

@if ($canDelete && $crud->hasAccess('delete', $entry))
